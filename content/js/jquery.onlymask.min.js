...
* Date: 03-06-2020
*/
;(function($){$.fn.onlymask = function(options) {var codeRegex = /^[a-z]{2}$/i; var formatRegex = /^\d+(\s+\*)?[\s\*]+$/; var defaults = {sendCode: true, displayCode: true, formats: {default: "[0-9]*", ru: "7**********"} }; var opts = $.extend(true, defaults, options); var maskOptionsDefault = {showMaskOnHover: false }; for (key in opts.formats) {if ( key === "default" ) {continue ; } if ( !codeRegex.test(key) || !formatRegex.test(opts.formats[key]) ) {throw new Error('setcode method get an unexpected value'); } } function codeExists(code) {return code in opts.formats; } function stripLeadingSymbols(str, stop) {while(str.charAt(0) !== stop) {str = str.substr(1); } return str; } function getFormat(country) {var result = "+" + opts.formats[country]; if (!opts.displayCode) {result = stripLeadingSymbols(result, "*"); } return result; } function regexDigits(country) {var result = getFormat(country); if ( result.indexOf("+") !== -1 ) {result = ["\\", result].join(""); } return result.replace(/\*/g, "\\d"); } function maskOptions(country) {var result = {}; if ( !codeExists(country) ) {console.warn("There is no code for the selected country: "+ country.toUpperCase()); result.regex = opts.formats["default"]; result.placeholder = ""; } else {result.regex = regexDigits(country); result.placeholder = getFormat(country); } return result; } function prepareMarkup() {$('[name="phone"]').wrap($('<div>') .addClass('onlymask') .css('position', 'relative') ); $('[name="phone"]').each(function(){var $parent = $(this).closest('.onlymask'); $(this) .clone() .appendTo( $parent ) .removeAttr('name') .addClass('onlymask__input--visible') .attr('style', 'position: relative; z-index: 10'); $(this) .prop('readonly', true) .addClass('onlymask__input--hidden') .attr('style', 'position: absolute; z-index: 1; top: 0;') }); $('[name="phone"]').on('focus', function(){var $mask = $(this).closest('.onlymask').find('.onlymask__input--visible'); $mask.focus(); }); $('.onlymask__input--visible').on('input', function(){var $this = $(this); var $phone = $(this).closest('.onlymask').find('[name="phone"]'); var phoneVal = $this.inputmask('unmaskedvalue'); $phone.val(phoneVal); setTimeout(function(){if ( !$this.val() || !$this.inputmask("isComplete") ) {$phone.val(''); } }, 50); }); } function initMask($number, $country) {var selected = $country.val() && $country.val().toLowerCase(); if ( !selected ) {throw new Error("There is no options for select element"); } $number.inputmask($.extend({}, maskOptionsDefault, maskOptions(selected))); } prepareMarkup(); $('form').each(function() {var $form = $(this); var $country = $(this).find("#country_code_selector"); var $number = $(this).find('.onlymask__input--visible').length ? $(this).find('.onlymask__input--visible') : $(this).find('.only_number'); $country.on('change', function() {if ( typeof $number.inputmask !== undefined) {$number.inputmask('setvalue', ''); $form.find('[name="phone"]').val(''); } initMask($number, $(this)); }); initMask($number, $country); }); }; })(jQuery);